...
preroi = list(pd.read_csv("../../../sorted_AAL_labels.txt")["label"].values) #nombre de las areas
...
klCNT = np.vstack([np.array([kl(occs[n,c,:],emp_occs_all[0]).sum() for c in range(len(Cs))]) for n in nodes])
klMCS = np.vstack([np.array([kl(occs[n,c,:],emp_occs_all[1]).sum() for c in range(len(Cs))]) for n in nodes])
klUWS = np.vstack([np.array([kl(occs[n,c,:],emp_occs_all[2]).sum() for c in range(len(Cs))]) for n in nodes])

# halt

# GoCNT,GoMCS,GoUWS = [Gs[np.argmin(a)] for a in (klCNT,klMCS,klUWS)]
whereCNT,whereMCS,whereUWS = np.where(klCNT==klCNT.min()),np.where(klMCS==klMCS.min()),np.where(klUWS==klUWS.min())
...
ceteris_paribus = {states[i]:(klCNT,klMCS,klUWS)[i][:,C_base_id] for i in range(3)}
no_move_id = int(np.where(Cs==0)[0])
no_move = {states[i]:(klCNT,klMCS,klUWS)[i][:,no_move_id].mean() for i in range(3)}
...
plt.hlines(no_move["CNT"],0,90,color="tab:red",linestyle="dashed",label="baseline")
plt.xticks(())
plt.legend()
...
plt.hlines(no_move["MCS"],0,90,color="tab:red",linestyle="dashed",label="baseline")
plt.xticks(())
plt.legend()
...
plt.hlines(no_move["UWS"],0,90,color="tab:red",linestyle="dashed",label="baseline")
...
def plot_with_color(x,ys,colorvec=Hin_node,label="Integration"):
    ...
    for n, node in enumerate(nodes):
        ...
        if node in important_nodes:
            plt.plot(x, ys[node], color=line_color, label=ROIlabels[node].split("-")[1])
        else:
...
fig = plt.figure(5)
plt.clf()
plt.suptitle("top: Hin node vs optimal fit to state, force below. Bottom: node sensitivity")


###variation vs Hin node
plt.subplot2grid((5,3),(0,0))
plt.scatter(Hin_node,mins_CNT)
plt.xlabel("Hin_node");plt.ylabel("mins KL")
plt.xscale("log")
...
plt.xlabel("Hin_node");plt.ylabel("mins EUC")
plt.xscale("log")
...
plt.xlabel("node strength");plt.ylabel("mins")
